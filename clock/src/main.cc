...
{
  float varr_in[8]
      = { 0, width / 2.0f, -width / 2.0f, 0, 0, -length, width / 2.0f, 0 };
  int varr_out[8];

  float matrix[2][2] = { { (float) cos(angle), (float) -sin(angle) },
...
display(Window *win)
{
  ...
  blit(clockface->pixels,
       Rect(0, 0, clockface->width, clockface->height),
       fb->pixels,
       get_screen_size(),
       rect.x,
       rect.y);
